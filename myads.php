...
?>
	...
	<h3>  My Ads <small class="pull-right">  <?php
	
	
	$query="select count(*) as cnt from ad where s_id='$sellerID' ";
	$result=mysql_query($query);
...
<div class="tab-content">
	...
	<div class="tab-pane  active" id="blockView">
		<ul class="thumbnails">
        <?php
		$query="SELECT * FROM ad WHERE s_id =  '$sellerID' ";
		$result=mysql_query($query);
				while($row=mysql_fetch_array($result)){
					$status=$row['status'];
					$ad_t=$row['ad_title'];
			...
			echo "<div class='caption'>";
				 echo "<h5>".$ad_t."</h5>";
				  echo "<p>".$time. " months used </p>";
				   echo "<h4 style='text-align:center'><a class='btn' href='product_details.php?ad=$ad_id'> <i class='icon-zoom-in'></i></a>  <a class='btn btn-primary' href='#'>".$price."</a>
				   <p>status: ".$status."</p> </h4>
				</div>
			  </div>
			</li>";
				}
